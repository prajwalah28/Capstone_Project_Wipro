...
import com.ApiGateway.apigateway.utils.JwtUtil;
...
public class JwtAuthenticationFilter extends AbstractGatewayFilterFactory<JwtAuthenticationFilter.Config> {
    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> {
            ...
            String userId = jwtUtil.extractClaim(token, claims -> 
            claims.get("userId") != null ? claims.get("userId").toString() : null
        );

        ServerHttpRequest modifiedRequest = request.mutate()
                .header("X-Auth-Username", jwtUtil.extractUsername(token))
                .header("X-Auth-Role", userRole)
                .header("X-Auth-UserId", userId != null ? userId : "") // 👈 FIXED
                .build();
